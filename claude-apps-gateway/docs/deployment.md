...
> `:2.1.251` in place, and re-running pass 2 with the same `-c imageTag` leaves the
...
example pins 2.1.251). Desktop then runs the same browser SSO and fetches its config from
