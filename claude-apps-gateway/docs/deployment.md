...
> `:2.1.272` in place, and re-running pass 2 with the same `-c imageTag` leaves the
...
example pins 2.1.272). Desktop then runs the same browser SSO and fetches its config from
