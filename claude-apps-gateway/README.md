...
No license or per-seat fee. A live deploy of this example idles at roughly **$188/month** (us-east-1, stack defaults): the largest line item is the six interface VPC endpoints (~$88), then Fargate (~$36), the NAT gateway (~$33), the ALB (~$16), and RDS (~$14). Amazon Bedrock inference is separate, charged at the same rates as calling Bedrock directly, and dominates at any real scale. See [`docs/costs.md`](docs/costs.md) for the breakdown and the posture trade-offs that reduce it.
