...
def get_bedrock_client():
    config = Config(
        ...
        retries={"total_max_attempts": 1000, "mode": "adaptive"},
    )
...
def add_chat():
    ...
    default_prompt = """IMPORTANT: ALWAYS VIEW YOUR MEMORY DIRECTORY BEFORE DOING ANYTHING ELSE.
MEMORY PROTOCOL:
...
3. Use memory_{topic}.md format to store general memories based on thier topic
4. ... (work on the task) ...
...
def render_chat_interface(chat_id, chat_data, bedrock_client):
    ...
    chat_data["system_prompt"] = st.text_input(
        "System Prompt:", value=chat_data["system_prompt"], key=f"system_{chat_id}"
    )
        ...
        with chat_container:
            ...
            if st.session_state.context_mgmt_enabled:
                ...
                with stream_container:
                    st.info(
                        f"⚙️ CM Config - Trigger: {context_mgmt['edits'][0]['trigger']['value']:,} tokens | Keep: {context_mgmt['edits'][0]['keep']['value']} tools | Clear≥: {context_mgmt['edits'][0]['clear_at_least']['value']:,} tokens"
                    )
                    ...
                    if event["type"] == "text":
                        ...
                        if "context_management" in usage:
                            ...
                            if applied_edits and len(applied_edits) > 0:
                                ...
                                with stream_container:
                                    st.success(
                                        f"🎉 Context Management Active! Cleared {total_cleared} old tool use(s) and saved {total_saved:,} tokens."
                                    )
                            else:
    ...
    # Clear button (small)
    if st.button("Clear Chat", key=f"clear_{chat_id}", type="secondary"):
        chat_data["messages"] = []
        if "last_usage" in chat_data:
...
DEFAULT_SYSTEM_PROMPT = """IMPORTANT: ALWAYS VIEW YOUR MEMORY DIRECTORY BEFORE DOING ANYTHING ELSE.
MEMORY PROTOCOL:
1. Store all memories in .md format at /home/claude/memories/; when a user identifies themselves, create or update memory_{username_sanitized}.md (replacing spaces/special chars with underscores)
2. Use the `view` command of your `memory` tool to check for earlier progress.
3. Use memory_{topic}.md format to store general memories based on thier topic
...
with st.expander("⚙️ Context Management Settings", expanded=False):
    ...
    **Context Editing** automatically clears old tool results when conversations grow large,
    while preserving memory files. This keeps multi-user workflows efficient.
        ...
        if st.session_state.context_mgmt_enabled:
            ...
            with col2a:
                st.session_state.context_trigger_threshold = st.slider(
                    ...
                    max_value=50000,
                    value=min(
                        max(st.session_state.context_trigger_threshold, 1000), 50000
                    ),
                    step=1000,
...
# Example workflows
with st.expander("💡 Example Multi-User Workflows", expanded=False):
    st.markdown(
        """
    ### Multi-User Sales Workflow Example:

    All chats use the same system prompt (default includes memory protocol).
    Users define their roles through their messages:

    **Chat 1 - Alice (Discovery):**
    ```
    I'm Alice from TechCorp. We're working with ACME Corp on a cloud migration project.
    Budget is $500K, timeline is 9 months, they want to migrate 50 applications.
    ```

    **Chat 2 - John (Solution Architect):**
    ```
...
if st.session_state.context_mgmt_enabled:
    ...
    with st.expander("📊 Context Management Statistics", expanded=False):
        if st.session_state.context_stats:
            ...
            st.info(
                """💡 **No context clears yet.** To see context management in action:
