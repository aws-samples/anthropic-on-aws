...
def filter_empty_content_blocks(content_blocks):
    """Remove empty text blocks from content to avoid API validation errors.

    The API requires all messages to have non-empty content except for the
    optional final assistant message. This filters out any text blocks with
    empty strings while preserving tool_use blocks.

    Args:
        content_blocks: List of content blocks from assistant response

    Returns:
        Filtered list with only non-empty text blocks and all tool_use blocks
    """
    filtered = []
    for block in content_blocks:
        if block["type"] == "text":
            # Only include text blocks with actual content
            if block.get("text", "").strip():
                filtered.append(block)
        else:
            # Always include non-text blocks (tool_use, etc.)
            filtered.append(block)
    return filtered
